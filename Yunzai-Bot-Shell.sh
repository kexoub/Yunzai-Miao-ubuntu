...
{	
	...
	echo -e "已安装喵喵、图鉴插件，机器人正常运行后输入【\033[34m#喵喵帮助\033[0m】、【\033[34m#图鉴帮助\033[0m】以分别查看帮助面板"
    echo -e '现在请输入\033[43;31mnode app\033[0m或\033[43;31myz\033[0m以启动机器人并进行配置'
    echo '结束，跑路'
    echo '赞美乐神'
    exit
}
...
#脚本多处使用figlet，干脆直接检测一下装上得了
	...
	clear
	echo '本脚本目前将部署并支持原云崽项目，但该项目已暂停更新，仅由喵喵插件作者例行日常维护，推荐部署喵崽（喵喵插件作者更新的云崽仓库）。'
	echo
	echo '且同时建议已部署云崽的用户进行喵崽的部署，喵崽部署将迁移您的部分数据'
	echo
	echo '请问您希望部署什么内容？'
    echo '【1】*推荐*喵崽	【2】云崽'
	read -p '请键入数字后回车：' change
	if [ $change == 1 ];then
			bash <(curl -sL https://gitee.com/fw-cn/Yunzai/raw/master/Miao-Yunzai-Shell.sh)
	elif [ $change == 2 ];then
		Yunzai
		YunzaiNum
	else
		echo '您输入的内容不是可用选项，已退出脚本'
		exit
	fi		
else
    echo '当前运行环境不支持！'
